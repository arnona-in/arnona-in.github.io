...
const celebrate = () => {
  ...
  if (isBirthday.value) {
    ;[262, 262, 294, 262, 349, 330, 262, 262, 294, 262, 392, 349, 262, 262, 523, 440, 349, 330, 294, 392, 349, 392, 440, 523].forEach((note, index) => playTone(note, 0.3, index * 0.27))
  } else {
    playTone(523, 0.2)
...
<template>
  <section class="for-you-page">
    ...
    <span v-for="piece in confetti" :key="piece.id" class="confetti" :style="{ left: piece.left, animationDelay: piece.delay, animationDuration: piece.duration, background: piece.color, transform: `rotate(${piece.rotate})` }"></span>
    <div v-if="isBirthday && explored" class="balloon-field" aria-hidden="true">
      <span class="balloon balloon-pink"></span><span class="balloon balloon-yellow"></span><span class="balloon balloon-green"></span>
    </div>
    ...
    <div class="for-you-card">
      ...
      <div v-else-if="isBirthday" class="for-you-message birthday-message">
        ...
        <button v-if="!explored" class="for-you-button" @click="celebrate">Open your birthday wish <span>✦</span></button>
        <div v-else class="birthday-reveal">
          <div class="birthday-number">26</div>
          <p class="after-message">Twenty-six in 2026 — a special little alignment, made just for you. 🎂</p>
          <p class="celebration-line">Today, the whole sky is cheering for you, Umang.</p>
        </div>
      </div>
...
.for-you-orb { position: absolute; border-radius: 50%; filter: blur(1px); opacity: .65; }.orb-one { width: 350px; height: 350px; top: -130px; left: -90px; background: #f2c7ba; }.orb-two { width: 420px; height: 420px; bottom: -210px; right: -140px; background: #d5e4ce; }.confetti { position: absolute; z-index: 4; top: -20px; width: 8px; height: 17px; animation: fall 6s ease-in forwards; }
.birthday-reveal { animation: birthday-reveal .9s cubic-bezier(.2,.8,.2,1) both; }.birthday-number { color: #d87862; font: 700 74px/.8 'Playfair Display', serif; text-shadow: 3px 4px 0 #f4d494; animation: number-pop 1.4s ease-in-out infinite alternate; }.celebration-line { margin: 17px auto 0; color: #29473e; font: 500 18px/1.4 'Playfair Display', serif; }.balloon-field { position: absolute; z-index: 1; inset: 0; pointer-events: none; }.balloon { position: absolute; bottom: -90px; width: 70px; height: 88px; border-radius: 50% 50% 48% 48%; animation: balloon-rise 9s ease-in-out infinite; }.balloon::after { content: ''; position: absolute; width: 1px; height: 160px; top: 82px; left: 50%; background: rgba(86, 91, 80, .45); }.balloon-pink { left: 7%; background: #e29b9b; animation-delay: .2s; }.balloon-yellow { right: 8%; width: 58px; height: 74px; background: #ebc26f; animation-delay: 1.4s; }.balloon-green { left: 88%; width: 47px; height: 62px; background: #91b5a0; animation-delay: 2.1s; }
@keyframes card-in { from { opacity: 0; transform: translateY(18px) scale(.98); } to { opacity: 1; transform: none; } } @keyframes float { 50% { transform: translateY(-8px) rotate(8deg); } } @keyframes reveal { from { opacity: 0; transform: translateY(12px); } to { opacity: 1; transform: none; } } @keyframes birthday-reveal { from { opacity: 0; transform: scale(.82) translateY(15px); } to { opacity: 1; transform: scale(1) translateY(0); } } @keyframes number-pop { to { transform: scale(1.08) rotate(-3deg); } } @keyframes fall { to { top: 105%; transform: translateY(0) rotate(540deg); } } @keyframes balloon-rise { 0% { transform: translateY(0) rotate(-4deg); } 50% { transform: translateY(-410px) rotate(7deg); } 100% { transform: translateY(-760px) rotate(-5deg); } }
